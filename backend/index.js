...
//creating endpoint for users logins
...
app.get('/popularinwomwns',async (req , res )=>{
...
const fetchUser = async (req , res , next)=>{
    ...
    if (!token) {
        res.status(401).send({errors:"Please aythenticate with valid token!"})
    }
    else {
        try {
...
// creating endpoint for related froducts

app.get('/related',async( req , res )=>{
    let products = await Product.find({})
    let related = products.slice(1).slice(-4);
    console.log("Rekated fetched!");
    res.send(related);
})
...
app.post('/addtocart', fetchUser , async (req , res)=>{
    console.log("adedd",req.body.itemId)
